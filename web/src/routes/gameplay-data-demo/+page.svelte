<script lang="ts">
	...
	const cursorTrail = $derived.by(() => cursorTrailPoints(selected, currentMs));
	...
	function latestCursor(session: Session | null, timestampMs: number) {
		...
		for (let index = session.events.length - 1; index >= 0; index -= 1) {
			...
			return normalizeCursor(session, event);
		}
		return null;
	}

	function cursorTrailPoints(session: Session | null, timestampMs: number) {
		if (!session) return [];
		const points: Array<{ left: number; top: number; age: number }> = [];
		for (const event of session.events) {
			if (event.timestampMs < timestampMs - 2200 || event.timestampMs > timestampMs || event.type !== 'mousemove') continue;
			const point = normalizeCursor(session, event);
			if (point) points.push({ ...point, age: clamp((timestampMs - event.timestampMs) / 2200, 0, 1) });
		}
		return points.slice(-10);
	}

	function normalizeCursor(session: Session, event: InputEvent) {
		...
		{:else}
			<div class="viewer-grid">
				<div class="player-card">
					...
					<div class="inputs">
						<div><p class="label">Keyboard</p><div class="keyboard-map">{#each keyboardRows as row}<div>{#each row as key}<span class:active={activeKeys.has(key)}>{key}</span>{/each}</div>{/each}</div></div>
						<div><p class="label">Mouse</p><div class="mouse-device"><div class="mouse-button left-button" class:active={activeMouse.has('Left')}>Left</div><div class="mouse-button wheel-button" class:active={activeMouse.has('Middle')}>Wheel</div><div class="mouse-button right-button" class:active={activeMouse.has('Right')}>Right</div><div class="mouse-palm"></div></div></div>
						<div><p class="label">Cursor</p><div class="cursor-grid">{#each cursorTrail as point, index (`${point.left}-${point.top}-${index}`)}<span class="trail-dot" style={`left:${point.left}%;top:${point.top}%;opacity:${Math.max(.18, 1 - point.age)}`}></span>{/each}{#if cursor}<span class="cursor-dot" style={`left:${cursor.left}%;top:${cursor.top}%`}></span>{/if}</div></div>
					</div>
					<div class="timeline">
